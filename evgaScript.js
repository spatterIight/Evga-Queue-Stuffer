...
// Variables
let parsedConfig = parseConfig()[0];
let parsedProxies = parseConfig()[1];

puppeteer.use(
  RecaptchaPlugin({
    provider: {
      id: '2captcha',
      token: parsedConfig.userinfo['2captcha'],
    },
    visualFeedback: true,
  })
)
...
console.log("                      / ____|         (_)     | |                         ");
console.log(" _____   ____ _  __ _| (___   ___ _ __ _ _ __ | |_   ");
console.log("/ _ \\ \\ / / _` |/ _` |\\___ \\ / __| '__| | '_ \\| __| ");
console.log("|  __/\ V / (_| | (_| |____) | (__| |  | | |_) | |_ ");
console.log("\\___| \\_/ \\__, |\\__,_|_____/ \\___|_|  |_| .__/ \\__| ");
console.log("           __/ |                        | |                    ");
console.log("          |___/                         |_|                   ");
...
async function main() {
  try {
    for (var index = 0; index < parsedConfig.userinfo.entries; index++) {
      ...
      let ChromeInstIncognito = await instantiateIncognito(ChromeInst, parsedConfig.userinfo.link, proxyUser, proxyPass);
      await notifyQueue(ChromeInstIncognito);
      await new Promise((resolve) => setTimeout(resolve, 1800000));
      await ChromeInstIncognito.close();
...
function parseConfig() {
  try {
    ...
    return [parsedConfig, parsedProxies];
  } catch (error) {
    console.log(error.message);
...
async function instantiateChrome(proxyAddr, proxyPort) {
  try {
    ...
    if(proxyAddr != null && proxyPort != null) {
      chromeArgs = ['--disable-web-security', '--disable-features=IsolateOrigins,site-per-process', '--window-position=99999,99999', '--proxy-server=' + proxyAddr + ':' + proxyPort + ''];
    }
...
async function notifyQueue(ChromeInstIncognito) {
  const PageInst = await ChromeInstIncognito.pages();
  let firstName = await getFirst();
  let lastName = await getLast();
  let emailAddr = firstName + lastName + parsedConfig.userinfo.catchall;
  await new Promise((resolve) => setTimeout(resolve, 2000)); /* make sure page is loaded */
  let notifyButtons = await PageInst[0].$$('a.btnAutoNotify')
  for (var index = 0; index < notifyButtons.length; index++) {
    await notifyButtons[index].click();
    await new Promise((resolve) => setTimeout(resolve, 5000)); /* wait for prompt */
    for (const frame of PageInst[0].mainFrame().childFrames()) {
      await frame.solveRecaptchas()
    }
    let iframe = await PageInst[0].waitForXPath('//iframe[@name="OnhandWin"]');
    let contentFrame = await iframe.contentFrame();
    let firstNameInput = await contentFrame.waitForXPath('//input[@name="tbFirstName"]');
      await firstNameInput.type(firstName, { delay: 50 });
    let lastNameInput = await contentFrame.waitForXPath('//input[@name="tbLastName"]');
      await lastNameInput.type(lastName, { delay: 50 });
    let emailAddrInput = await contentFrame.waitForXPath('//input[@name="tbEmail"]');
      await emailAddrInput.type(emailAddr, { delay: 50 });
    let submitButton = await contentFrame.waitForXPath('//input[@name="btnSubmit"]');
...
async function getFirst() {
  ...
  return firstNames[await getRandomInt(firstNames.length)];
}
...
async function getLast() {
  ...
  return lastNames[await getRandomInt(lastNames.length)];
}